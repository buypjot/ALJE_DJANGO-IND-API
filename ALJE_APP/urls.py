...
urlpatterns = [
  ...
  path('update-scan-status/<str:req_no>/<str:pick_id>/<str:customer_no>/<str:customer_site>/<str:new_status>/', 
    ...
    re_path(r'^update_employee/(?P<emp_id>\d+)/?$', update_employee, name='update_employee'),
     ...
     path('Generate_dispatch_print/', views.Generate_dispatch_print, name='Generate_dispatch_print'),

  # re_path(
    #     r'^Generate_dispatch_print‡(?P<deliveryno>[^‡]*)‡(?P<region>[^‡]*)‡(?P<transportor_Name>[^‡]*)‡(?P<vehicleNo>[^‡]*)‡(?P<driverName>[^‡]*)‡(?P<driverMobileNo>[^‡]*)‡(?P<date>[^‡]*)‡(?P<customerNo>[^‡]*)‡(?P<customername>[^‡]*)‡(?P<customersite>[^‡]*)‡(?P<deliveryaddress>[^‡]*)‡(?P<remmarks>[^‡]*)‡(?P<salesmanremmarks>[^‡]*)‡(?P<itemtotalqty>[^‡]*)‡(?P<products_param>.+)/?$',
    #     views.Generate_dispatch_print,
    #     name='Generate_dispatch_print'
    # ),
    # re_path(
    #     r'^Generate_picking_print‡(?P<pickid>[^‡]*)‡(?P<reqno>[^‡]*)‡(?P<region>[^‡]*)‡(?P<pickmanname>[^‡]*)‡(?P<deliveryaddress>[^‡]*)‡(?P<date>[^‡]*)‡(?P<customerNo>[^‡]*)‡(?P<customername>[^‡]*)‡(?P<customersite>[^‡]*)‡(?P<itemtotalqty>[^‡]*)‡(?P<products_param>.+)/?$',
    #     views.Generate_picking_print,
    #     name='Generate_picking_print'
    # ),
     path('Generate_picking_print/', views.Generate_picking_print, name='Generate_picking_print'),

     path('Generate_dispatch_details_print/', views.Generate_dispatch_details_print, name='Generate_dispatch_details_print'),

    re_path(
        r'^Generate_Shipment_dispatch_print‡(?P<shipmentnum>[^‡]*)‡(?P<receiptnum>[^‡]*)‡(?P<shipmentid>[^‡]*)‡(?P<transportor_Name>[^‡]*)‡(?P<vehicleNo>[^‡]*)‡(?P<driverName>[^‡]*)‡(?P<driverMobileNo>[^‡]*)‡(?P<date>[^‡]*)‡(?P<remarks>[^‡]*)‡(?P<itemtotalqty>[^‡]*)‡(?P<products_param>.+)/?$',
        views.Generate_Shipment_dispatch_print,
        name='Generate_Shipment_dispatch_print'
    ),
# re_path(
#     r'^Return_invoice_print/(?P<uniqulastreqno>[^/]+)/(?P<remarks>[^/]+)/(?P<superuserno>[^/]+)/(?P<superusername>[^/]+)/(?P<orgid>[^/]+)/(?P<date>[^/]+)/(?P<salesmano>[^/]+)/(?P<customerNo>[^/]+)/(?P<customername>[^/]+)/(?P<customersite>[^/]+)/(?P<invoiceno>[^/]+)/(?P<itemtotalqty>[^/]+)/(?P<products_param>.+)/$',
#     views.Return_invoice_print,
#     name='Return_invoice_print'
# ),
...
path(
    ...
    # path(
    #     'filteredProductcodeGetView/<str:itemcode>/',
...
path(
    ...
    path(
        ...
        update_quick_bill_access,
        name='update_quick_bill_access'
    ),

     path(
        'get-quick-bill-enable/',
        get_quick_bill_enable_status_Employeeid,
