...
from ALJE_APP.models import ShipmentID_Models
 
def generateshipment_id_helper():
    """
    Helper function to generate a new Shipment ID.
    Returns the created ShipmentID_Models object.
    """
    return ShipmentID_Models.objects.create()
            ...
            try:
                with transaction.atomic():
                    ...
                    with connection.cursor() as cursor:
                        ...
                        cursor.execute("""
                            SELECT TOP 1 [SHIPMENT_ID]
                            FROM [BUYP].[dbo].[WHR_INTER_ORG_DETIALS_TBL] WITH (NOLOCK)
...
def update_user_details(request, user_id):
    if request.method == 'PUT':
        try:
            ...
            # -------- CALL LOGIN UPDATE (SYNC) --------
            
            # Prepare payload for login_dynamic
            # Decrypt/Decode password to normal format for session login
            try:
                normal_password = decoded_password.decode('utf-8')
            except Exception:
                # Fallback if decoding fails
                normal_password = str(decoded_password)

            login_payload = {
                "type": "update", # Changed to update to ensure password syncs
            ...
            class InternalRequest:
                def __init__(self, meta, body_bytes):
                    self.method = 'POST'
                    self.META = meta
            ...
            mock_req = InternalRequest(request.META, json.dumps(login_payload).encode('utf-8'))
            login_dynamic(mock_req)
...
from Location_Map_App.views.session_view import login_dynamic
...
def create_user_details(request):
    if request.method == 'POST':
        try:
            ...
            # Decrypt/Decode password to normal format for session login
            try:
                normal_password = password_bytes.decode('utf-8')
            except Exception:
                # Fallback if decoding fails, though it shouldn't if it was valid base64 of a string
                normal_password = str(password_bytes)
...
#                 ])
#             # -------- CALL LOGIN INSERT --------
#             from Location_Map_App.session_view.login_dynamic import login_dynamic_service
#             client_ip = request.META.get('REMOTE_ADDR')

#             login_result = login_dynamic_service(
#                 op_type="insert",
#                 username=employee_username,
#                 password=encoded_password,  # or decoded if required
#                 client_ip=client_ip
#             )
...
def update_user_password(request, user_id):
    if request.method == 'PUT':
        try:
            ...
            # Execute raw SQL update and fetch username
            # We need the username to sync with the session/login table
            employee_username = None
            with connection.cursor() as cursor:
                # First, fetch the username (EMP_USERNAME) for this ID
                # The user request mentioned fetching EMPLOYEE_ID, but create_user_details uses EMP_USERNAME for login.
                # We will fetch EMP_USERNAME to ensure consistency with the login system.
                cursor.execute("SELECT EMP_USERNAME FROM WHR_USER_MANAGEMENT WHERE ID = %s", [user_id])
                row = cursor.fetchone()
                if row:
            ...
            if employee_username:
                login_payload = {
                    "type": "update", # Using 'update' type as this is a password update
                    "username": employee_username,
                    "password": normal_password
                }
                ...
                # Call the view with the mock request
                # We need to import login_dynamic if not already imported in this scope, 
                # but it is imported at module level in create_user_details fix. 
                # To be safe, we rely on the top-level import or import inside function if needed.
                # Since we added `from Location_Map_App.views.session_view import login_dynamic` at top in previous step, we can use it.
                mock_req = InternalRequest(request.META, json.dumps(login_payload).encode('utf-8'))
                login_dynamic(mock_req)
        ...
        query = '''
            ;WITH u_agg AS (
                ...
                WHERE
                    ...
                    AND u.INVOICE_NUMBER NOT LIKE 'R%%'
                    AND (u.QUANTITY - ISNULL(u.RETURN_QTY, 0)) >= u.DISPATCH_QTY
                    AND (u.QUANTITY - ISNULL(u.RETURN_QTY, 0)) <> 0
                GROUP BY
...
# @csrf_exempt
# @api_view(['POST'])
# def process_dispatch_request(request):
#     """
#     Process dispatch request with Optimised Bulk Execution.
#     - Eliminates N+1 Query problem.
#     - Pre-fetches Existence, Validation, and Manager Data.
#     - Uses executemany for batch inserts/updates.
#     - Returns Last Inserted PICK_ID (or 'Last No Pickid').
#     """
#     try:
#         data = request.data
#         if not data:
...
#         physical_warehouse = data.get("PHYSICAL_WAREHOUSE")
#         org_id = data.get("ORG_ID")
#         org_name = data.get("ORG_NAME")
#         salesman_no = data.get("SALESMAN_NO")
...
#         # Items List
#         table_datas = data.get("TABLE DATAS", [])
#         if not table_datas and "TABLE_DATAS" in data:
#              table_datas = data["TABLE_DATAS"]
             
#         if not table_datas:
#              return JsonResponse({'status': 'success', 'processed': 0, 'skipped': 0, 'message': 'No items to process'})

#         current_time = timezone.now()
#         client_ip = request.META.get("REMOTE_ADDR", "")
        
#         # ---------------------------------------------------------
#         # 1️⃣ Collect Keys for Bulk Fetching
#         # ---------------------------------------------------------
#         valid_items = [item for item in table_datas if item.get("undel_id")]
        
#         # Keys for Logic
#         undel_ids = [str(item["undel_id"]) for item in valid_items]
...
#             # SQL IN clause needs placeholders count matches
#             # If valid_items is huge (e.g. 2000+), we might need batching. 
#             # Assuming payload size is reasonable (e.g. <1000 items).
            
#             undel_placeholders = ','.join(['%s'] * len(undel_ids))
#             query_params_undel = undel_ids
            
#             # A. Existence Map (for Upsert)
#             existing_records = set()
#             with connection.cursor() as cursor:
#                  sql_exists = f"""
#                     SELECT REQ_ID, PICK_ID, UNDEL_ID 
#                     FROM [BUYP].[dbo].[WHR_DISPATCH_REQUEST]
#                     WHERE REQ_ID = %s 
#                     AND PICK_ID = %s 
#                     AND UNDEL_ID IN ({undel_placeholders})
#                  """
#                  cursor.execute(sql_exists, [req_id, pick_id] + query_params_undel)
#                  rows = cursor.fetchall()
#                  for r in rows:
#                      existing_records.add((str(r[0]), str(r[1]), str(r[2])))
...
#                         req.REQ_ID,
#                         req.UNDEL_ID,
#                         MAX(TRY_CAST(req.DISPATCHED_QTY AS INT)) AS dispatch_qty,
#                         (
#                             SUM(TRY_CAST(req.PICKED_QTY AS INT))
#                             + ISNULL(MAX(saved.saved_qty), 0)
#                             + ISNULL(MAX(truck.trucked_qty), 0)
#                         ) AS final_qty
                    
#                     FROM [BUYP].[dbo].[WHR_DISPATCH_REQUEST] req
#                     LEFT JOIN (
#                         SELECT req_no, Udel_id, SUM(TRY_CAST(Send_qty AS INT)) AS saved_qty
#                         FROM [BUYP].[dbo].[WHR_SAVE_TRUCK_DETAILS_TBL]
#                         WHERE PICK_ID NOT LIKE 'PK%%'
#                         GROUP BY req_no, Udel_id
#                     ) saved ON saved.req_no = req.REQ_ID AND saved.Udel_id = req.UNDEL_ID
#                     LEFT JOIN (
#                         SELECT REQ_ID, UNDEL_ID, SUM(TRY_CAST(TRUCK_SEND_QTY AS INT)) AS trucked_qty
#                         FROM [BUYP].[dbo].[WHR_TRUCK_SCAN_DETAILS]
#                         WHERE PICK_ID NOT LIKE 'PK%%'
#                         GROUP BY REQ_ID, UNDEL_ID
#                     ) truck ON truck.REQ_ID = req.REQ_ID AND truck.UNDEL_ID = req.UNDEL_ID
                    
#                     WHERE req.REQ_ID = %s 
#                     AND req.UNDEL_ID IN ({undel_placeholders})
#                     GROUP BY req.REQ_ID, req.UNDEL_ID
#                 """
#                 cursor.execute(sql_validate, [req_id] + query_params_undel)
#                 rows = cursor.fetchall()
#                 for r in rows:
#                     key = (str(r[0]), str(r[1])) 
#                     validation_map[key] = {
#                         'dispatch_qty': float(r[2] or 0), 
...
#             with connection.cursor() as cursor:
#                 sql_manager = f"""
#                     SELECT REQ_ID, UNDEL_ID, DISPATCHED_BY_MANAGER
#                     FROM [BUYP].[dbo].[WHR_CREATE_DISPATCH]
#                     WHERE REQ_ID = %s
#                     AND UNDEL_ID IN ({undel_placeholders})
#                 """
#                 cursor.execute(sql_manager, [req_id] + query_params_undel)
#                 rows = cursor.fetchall()
#                 for r in rows:
#                     key = (str(r[0]), str(r[1]))
#                     manager_map[key] = float(r[2] or 0)

#             # ---------------------------------------------------------
#             # 3️⃣ Logic Processing
#             # ---------------------------------------------------------
#             batch_upsert_updates = []
#             batch_inserts = []
#             batch_manager_updates = []
            
#             for item in valid_items:
#                 undel_id = str(item.get("undel_id"))
#                 send_qty = float(item.get("sendqty") or 0)
#                 if send_qty < 0: send_qty = 0
                
#                 upsert_key = (str(req_id), str(pick_id), str(undel_id))
#                 validation_key = (str(req_id), str(undel_id))
                
#                 # Check Upsert
#                 if upsert_key in existing_records:
#                     batch_upsert_updates.append([
#                         send_qty, item.get("Status"), current_time, created_by, flag_val,
...
#                 # Check Validation
#                 can_insert = True
#                 if validation_key in validation_map:
#                     v_data = validation_map[validation_key]
#                     d_qty = v_data['dispatch_qty']
#                     f_qty = v_data['final_qty']
#                     # logic: if dispatched == final -> skip.
#                     if d_qty == f_qty: can_insert = False
#                     elif d_qty == 0 and f_qty == 0: can_insert = False
#                     elif not (f_qty < d_qty and d_qty > 0): can_insert = False
...
#                 # Check Manager
#                 if validation_key not in manager_map:
#                     continue
                    
#                 mgr_qty_avail = manager_map[validation_key]
#                 qty_to_process = 0
#                 new_mgr_qty = 0
                
#                 if send_qty > mgr_qty_avail:
#                     qty_to_process = mgr_qty_avail
#                     new_mgr_qty = 0
#                 else:
#                     qty_to_process = send_qty
#                     new_mgr_qty = mgr_qty_avail - send_qty
                    
#                 batch_manager_updates.append([new_mgr_qty, req_id, undel_id])
                
#                 batch_inserts.append([
#                     pick_id, req_id, date_val, assign_pickman, physical_warehouse,
#                     org_id, org_name, salesman_no, salesman_name, manager_no, manager_name,
#                     customer_number, customer_name, customer_site_id,
#                     item.get("invoicedate"), item.get("invoiceno"),
#                     item.get("customer_trx_id"), item.get("customer_trx_line_id"),
#                     item.get("id") or 0, item.get("itemcode"), item.get("itemdetails"),
#                     item.get("invoiceQty"), item.get("totaldisreqqty"), item.get("balanceqty"),
#                     qty_to_process, 0, item.get("Status"),
#                     creation_date or current_time, created_by, client_ip, flag_val, undel_id
#                 ])
#                 success_count += 1
...
#             # 4️⃣ Batch Execution
#             # ---------------------------------------------------------
#             if (batch_upsert_updates or batch_manager_updates or batch_inserts):
#                 with transaction.atomic():
#                     with connection.cursor() as cursor:
#                         if batch_upsert_updates:
#                             cursor.executemany("""
#                                 UPDATE [BUYP].[dbo].[WHR_DISPATCH_REQUEST]
#                                 SET PICKED_QTY = %s, STATUS = %s, LAST_UPDATE_DATE = %s, LAST_UPDATED_BY = %s, FLAG = %s
#                                 WHERE REQ_ID = %s AND PICK_ID = %s AND UNDEL_ID = %s
#                             """, batch_upsert_updates)
                        
#                         if batch_manager_updates:
#                             try:
#                                 cursor.executemany("""
#                                     UPDATE [BUYP].[dbo].[WHR_CREATE_DISPATCH]
...
#                                 print(f"Manager update failed: {e_mgr}")
#                                 # Continue? Transaction atomic will rollback anyway.
#                                 raise e_mgr
                        
#                         if batch_inserts:
#                             cursor.executemany("""
#                                 INSERT INTO [BUYP].[dbo].[WHR_DISPATCH_REQUEST] (
#                                     PICK_ID, REQ_ID, DATE, ASSIGN_PICKMAN, PHYSICAL_WAREHOUSE,
#                                     ORG_ID, ORG_NAME, SALESMAN_NO, SALESMAN_NAME, MANAGER_NO, MANAGER_NAME,
#                                     CUSTOMER_NUMBER, CUSTOMER_NAME, CUSTOMER_SITE_ID, 
#                                     INVOICE_DATE, INVOICE_NUMBER, CUSTOMER_TRX_ID, CUSTOMER_TRX_LINE_ID,
#                                     LINE_NUMBER, INVENTORY_ITEM_ID, ITEM_DESCRIPTION, 
#                                     TOT_QUANTITY, DISPATCHED_QTY, BALANCE_QTY, PICKED_QTY, SCANNED_QTY,
#                                     STATUS, CREATION_DATE, CREATED_BY, CREATED_IP, FLAG, UNDEL_ID
#                                 ) VALUES (
...
#         # ---------------------------------------------------------
#         # 5️⃣ Return Last Checked PICK_ID
#         # ---------------------------------------------------------
#         last_pick_status = "No Pickid"
#         if pick_id:
#             with connection.cursor() as cursor:
#                 # Check if this specific PICK_ID exists in the table now
#                 cursor.execute("SELECT TOP 1 PICK_ID FROM [BUYP].[dbo].[WHR_DISPATCH_REQUEST] WHERE PICK_ID = %s", [pick_id])
#                 row = cursor.fetchone()
#                 if row and row[0]:
#                     last_pick_status = f"{row[0]}"

#         return JsonResponse({
#             'status': 'success', 
#             'processed': success_count, 
#             'skipped': skipped_count,
#             'PICK_ID': last_pick_status
#         })
...
def _generate_next_pk_id_helper():
    """
    Internal helper to generate the next unique PK ID.
    Format: PKYYMM01, PKYYMM02...
    Returns the string PK ID.
    """
    while True:
        try:
            ...
            prefix = f"PK{year_str}{month_str}" # e.g., 'PK2512'
 
            # 2. Find the latest sequence number for this Month/Year
            last_entry = PK_UniqId_tbl.objects.filter(
                PK__startswith=prefix
            ).annotate(
                text_len=Length('PK')
            ).order_by('-text_len', '-PK').first()
 
            if last_entry:
                # Extract the part after the prefix
                last_no = last_entry.PK
...
@api_view(['GET'])
def PK_UNIQE_ID_Next_Id(request):
    """
    View to generate the next unique PK ID.
    Uses the helper function.
    """
    try:
        new_pk_id = _generate_next_pk_id_helper()
        return Response({
            "status": "success",
            "PK_ID": new_pk_id
        }, status=status.HTTP_200_OK)
...
def process_dispatch_request(request):
    ...
    try:
        ...
        # Header Details
        pick_id = _generate_next_pk_id_helper() # Auto-generated ID (PK...)
        req_id = data.get("REQ_ID")
        date_val = data.get("DATE")
        assign_pickman = data.get("ASSIGN_PICKMAN")
        physical_warehouse = data.get("PHYSICAL_WAREHOUSE")
        ...
        if valid_items:
            ...
            for item in valid_items:
                ...
                batch_inserts.append([
                    ...
                    item.get("invoicedate"), item.get("invoiceno"),
                    item.get("customer_trx_id"), item.get("customer_trx_line_id"),
                    item.get("linenumber") or 0, item.get("itemcode"), item.get("itemdetails"),
                    item.get("invoiceQty"), item.get("totaldisreqqty"), item.get("balanceqty"),
                    qty_to_process, 0, item.get("Status"),
                    creation_date or current_time, created_by, client_ip, flag_val, undel_id
                ])
        ...
        try:
            ...
            insert_sql = """
                INSERT INTO WHR_PICKED_MAN (
                    ...
                    LAST_UPDATE_IP, FLAG, UNDEL_ID,ITEM_LOCATION_CODE,ITEM_LOCATION_BARCODE
                ) VALUES (
                    %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                    %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
                    %s,%s,%s,%s,%s,%s,%s,%s,%s,%s,
            ...
            params = [
                ...
                row.get("LAST_UPDATE_IP"), row.get("FLAG"), row.get("UNDEL_ID"),row.get("ITEM_LOCATION_CODE"), row.get("ITEM_LOCATION_BARCODE")
            ]
                ...
                def run_update():
                    try:
                        UpdateDispatchQtyView.update_qty(
                         ...
                         # 2. NEW STOCK STATUS UPDATE
                        with connection.cursor() as cursor:
                          cursor.execute(f"""
                     ;WITH OrderedRows AS (
                        SELECT TOP ({final_to_insert}) ID
                        FROM WHR_Stock_Details_tbl
                        WHERE
                            Item_Code = %s
                            AND Location_Barcode = %s
                            AND Location_Code = %s
                            AND ISNULL(Stock_status, '') NOT IN ('OnProgress', 'Dispatched')
                        ORDER BY ID DESC
                    )
                    UPDATE WHR_Stock_Details_tbl
                    SET
                        Attribute1 = %s,
                        Stock_status = 'OnProgress'
                    WHERE ID IN (SELECT ID FROM OrderedRows);                              
                         """, [
                            row["INVENTORY_ITEM_ID"],
                            row["ITEM_LOCATION_BARCODE"],
                            row["ITEM_LOCATION_CODE"],
                            row["PICK_ID"]
                         ])
...
class filteredProductcodeGetView(viewsets.ViewSet):
    """
    Retrieve products based on itemcode,
    handling SERIAL_STATUS and PRODUCT_BARCODE rules correctly.
    """

    def list(self, request, itemcode=None, DESCRIPTION=None):

        queryset = ProductcodeGetModels.objects.filter(
            ITEM_CODE=itemcode,
        ...
        if not queryset.exists():
            return Response(
                {'message': 'No products found for the given itemcode.'},
                status=404
            )

        # --------------------------------------------------
        # RULE 1: BYPASS CONDITION
        # PRODUCT_BARCODE = 0 / 00 / NULL
        # AND SERIAL_STATUS = N OR NULL
        # --------------------------------------------------
        bypass_items = queryset.filter(
            (
                Q(PRODUCT_BARCODE__in=['0', '00']) |
                Q(PRODUCT_BARCODE__isnull=True)
            ) &
            (
                Q(SERIAL_STATUS='N') |
                Q(SERIAL_STATUS__isnull=True)
            )
        )

        if bypass_items.exists():
            return Response(
                {'message': 'Bypass: Scanning not required for this item.'},
                status=200
            )

        # --------------------------------------------------
        # RULE 2: OTHERWISE SHOW DETAILS
        # --------------------------------------------------
        serializer = ProductcodeGetserializers(queryset, many=True)
...
# class UpdateScanStatus_Direct_LoadmanView(View):
#     def post(self, request, *args, **kwargs):
#         return self.handle_request(request, *args, **kwargs)
...
class UpdateScanStatus_Direct_LoadmanView(View):
    ...
    def post(self, request, req_no, pick_id, customer_no, customer_site, new_status):
        try:
            new_status = new_status.replace("%20", " ")
            ...
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    for item in items:
                        undel_id = item.get("undel_id")
                        item_code = item.get("item_code")
                        qty = int(item.get("qty", 0))

                        if not undel_id or not item_code or qty <= 0:
                            continue
                        ...
                        barcode = row[0] if row[0] not in (None, '') else '00'

                        # 1️⃣ Update already-filled rows (status only)
                        cursor.execute("""
                            UPDATE TOP (%s) BUYP.dbo.WHR_SAVE_TRUCK_DETAILS_TBL
                            SET SCAN_STATUS=%s
                            WHERE req_no=%s
                              AND pick_id=%s
                              AND Customer_no=%s
                              ...
                              AND Udel_id=%s
                              AND PRODUCT_CODE<>'empty'
                              AND SERIAL_NO<>'empty' and SCAN_STATUS != 'Request for Delivery'
                        """, [
                            qty, new_status,
                            req_no, pick_id,
                            customer_no, customer_site,
                            undel_id
                        ])

                        updated_filled = cursor.rowcount
                        remaining_qty = qty - updated_filled
                        total_updated += updated_filled

                        # 2️⃣ Update EMPTY → FILLED rows
                        updated_empty = 0
                        if remaining_qty > 0:
                            cursor.execute("""
                                UPDATE TOP (%s) BUYP.dbo.WHR_SAVE_TRUCK_DETAILS_TBL
                                ...
                                WHERE req_no=%s
                                  ...
                                  AND SERIAL_NO='empty' and SCAN_STATUS != 'Request for Delivery'
                            """, [
                                remaining_qty, barcode, new_status,
                                req_no, pick_id,
                                customer_no, customer_site,
                                undel_id
                            ])
                            ...
                            total_updated += updated_empty

                        # 3️⃣ 🔥 Update PICKED_MAN using ONLY updated_empty count
                        if updated_empty > 0:
                            cursor.execute("""
                                UPDATE TOP (%s) BUYP.dbo.WHR_PICKED_MAN
                                SET PRODUCT_CODE=%s,
                                    SERIAL_NO='LSSNSA'
                                WHERE REQ_ID=%s
                                  AND PICK_ID=%s
                                  AND UNDEL_ID=%s
                                  AND PRODUCT_CODE='empty'
                                  AND SERIAL_NO='empty' and 
                                    SERIAL_NO <>'LSSNSA'
                            """, [
                                updated_empty, barcode,
                                req_no, pick_id,
                                undel_id
                            ])
...
@csrf_exempt
def dispatch_progress_Invoice_details_raw_view(request):
    if request.method != 'GET':
        return JsonResponse({'status': 'error', 'message': 'Invalid request method'}, status=405)

    warehouse = request.GET.get('warehouse')
    status = request.GET.get('status')  # Progressing | Fullfilled
        ...
        with connection.cursor() as cursor:
            ...
            ;WITH DispatchData AS (
                ...
                WHERE d.FLAG NOT IN ('R','OU')
                  AND d.PHYSICAL_WAREHOUSE = %s
                GROUP BY
                    d.REQ_ID, d.UNDEL_ID, d.INVOICE_NUMBER,
                    d.INVENTORY_ITEM_ID, d.ITEM_DESCRIPTION,
                    d.COMMERCIAL_NO, d.COMMERCIAL_NAME,
                    d.SALESMAN_NO, d.SALESMAN_NAME,
                    d.CUSTOMER_NUMBER, d.CUSTOMER_NAME,
                    d.CUSTOMER_SITE_ID, d.INVOICE_DATE, d.DELIVERY_DATE
            ),
            TruckQty AS (
                SELECT
                    REQ_ID,
                    ...
                    ITEM_CODE AS INVENTORY_ITEM_ID,
                    COUNT(*) AS previous_truck_qty
                FROM BUYP.dbo.WHR_TRUCK_SCAN_DETAILS
                WHERE FLAG NOT IN ('R','SR','OU')
                GROUP BY REQ_ID, UNDEL_ID, INVOICE_NO, ITEM_CODE
            ),
            ReturnQty AS (
                SELECT
                    REQ_ID,
                ...
                WHERE RE_ASSIGN_STATUS <> 'Re-Assign-Finished'
                GROUP BY
                    REQ_ID, UNDEL_ID, INVOICE_NO, ITEM_CODE,
                    CUSTOMER_NUMBER, CUSTOMER_SITE_ID
            ),
            PickedQty AS (
                SELECT
                    REQ_ID,
                ...
                FROM BUYP.dbo.WHR_PICKED_MAN
                WHERE FLAG NOT IN ('R','OU')
                GROUP BY REQ_ID, UNDEL_ID, INVOICE_NUMBER, INVENTORY_ITEM_ID
            )
            SELECT
                dd.id,
                dd.REQ_ID,
            ...
            ORDER BY dd.REQ_ID, dd.UNDEL_ID, dd.INVOICE_NUMBER
            """
            ...
            for row in rows:
                ...
                # status filter
                if status == 'Progressing' and dis_qty == truck_qty:
                    continue
                if status == 'Fullfilled' and dis_qty != truck_qty:
                    continue
            ...
            return JsonResponse(list(result_map.values()), safe=False)
...
@csrf_exempt
def Generate_dispatch_details_print(request, dispatch_id):
    if request.method != 'GET':
    ...
    try:
        with connections['default'].cursor() as cursor:
            ...
            if not header:
                return JsonResponse(
                    {'error': f'No data found for DISPATCH_ID {dispatch_id}'},
                    status=404
                )
            ...
            cursor.execute("""
                SELECT
                    ...
                    ITEM_DETAILS,
                    PRODUCT_CODE,
                    ISNULL(SERIAL_NO, 'NoSR00') AS SERIAL_NO
                FROM WHR_TRUCK_SCAN_DETAILS
                WHERE DISPATCH_ID = %s
                ORDER BY INVOICE_NO, ITEM_CODE
            """, [dispatch_id])
        ...
        for row in rows:
            inv_no, item_code, item_details, productcode, serialno = row

            if not serialno or serialno.lower() == 'null':
                serialno = 'NoSR00'
            ...
            if key not in grouped:
                grouped[key] = {
                    ...
                    'serialnos_raw': [],
                    'serialnos_display': set(),
                }

            grouped[key]['serialnos_raw'].append(serialno)

            if serialno == 'NoSR00':
                grouped[key]['serialnos_display'].add('NoSR00')
            else:
                grouped[key]['serialnos_display'].add(serialno)
        ...
        for g in grouped.values():
            items.append({
                's_no': sno,
                'inv_No': g['inv_No'] or '',
                'item_code': g['item_code'] or '',
                'item_details': g['item_details'] or '',
                'productcode': g['productcode'] or '',
                'serialnos': ', '.join(sorted(g['serialnos_display'])),
                'grouped_serialnos': sorted(g['serialnos_display']),
                'total_serialnos': len(g['serialnos_raw']),
            })
            sno += 1
        ...
        deliveryaddress = header[11]
        salesmanremmarks = header[12]
        remmarks = header[13]
        ...
        # ---------- CONTEXT ----------
        context = {
            'deliveryno': header[0] or '',
            'date': formatted_date,
            'pickid': header[2] or '',
            'pickmanname': header[3] or '',
            'transportor_Name': header[4] or '',
            'vehicleNo': header[5] or '',
            'driverName': header[6] or '',
            'driverMobileNo': header[7] or '',
            'customerNo': header[8] or '',
            'customername': header[9] or '',
            'customersite': header[10] or '',
            # ✅ FIXED — cleaned values used
            'deliveryaddress': deliveryaddress,
            'salesmanremmarks': salesmanremmarks,
            'remmarks': remmarks,
            'region': header[14] or '',
            'itemtotalqty': sum(i['total_serialnos'] for i in items),
            'items': items,
        }
...
@csrf_exempt
def Generate_dispatch_print(request, dispatch_id):
    if request.method != 'GET':
        return JsonResponse({'error': 'Only GET method allowed'}, status=405)
    ...
    try:
        ...
        deliveryaddress = header[14]
        salesmanremmarks = header[15]
        remmarks = header[16]
        ...
        context = {
            ...
            'warehouse': header[4] or '',
            'region': header[4] or '',

            'customerNo': header[6] or '',
            'customername': header[7] or '',
            'customersite': header[8] or '',

            'transportor_Name': header[9] or '',
            'pickmanname': header[10] or '',
            'vehicleNo': header[11] or '',
            'driverName': header[12] or '',
            'driverMobileNo': header[13] or '',
...
@csrf_exempt
def Generate_picking_print(request, pickid):
    if request.method != 'GET':
        return JsonResponse({"error": "Only GET method allowed"}, status=405)
    ...
    try:
        with connections['default'].cursor() as cursor:
            cursor.execute("""
                SELECT
                    ...
                    wdr.DATE,
                    wdr.ASSIGN_PICKMAN,
                    wdr.PHYSICAL_WAREHOUSE,
                    wdr.ORG_NAME,
                    wdr.CUSTOMER_NUMBER,
                    wdr.CUSTOMER_NAME,
                    wdr.CUSTOMER_SITE_ID,
                    wdr.INVOICE_NUMBER,
                    wdr.INVENTORY_ITEM_ID,
                    wdr.ITEM_DESCRIPTION,
                    wdr.TOT_QUANTITY,
                    wdr.PICKED_QTY,
                    wdr.SCANNED_QTY,
                    wdr.BALANCE_QTY,
                    wdr.LINE_NUMBER,
                    wcd.DELIVERYADDRESS
                FROM WHR_DISPATCH_REQUEST wdr
                LEFT JOIN (
                    SELECT
                        REQ_ID,
                        MAX(DELIVERYADDRESS) AS DELIVERYADDRESS
                    FROM WHR_CREATE_DISPATCH
                    GROUP BY REQ_ID
                ) wcd ON wdr.REQ_ID = wcd.REQ_ID
                WHERE wdr.PICK_ID = %s
                ORDER BY wdr.LINE_NUMBER
        ...
        if not rows:
            return JsonResponse(
                {"error": f"No data found for PICK_ID {pickid}"},
                status=404
            )
        ...
        context = {
            ...
            "reqno": header[1],
            "date": formatted_date,
            "pickmanname": header[3],
            "warehouse": header[4],
            "region": header[5],

            'deliveryaddress': deliveryaddress,
            "customerNo": header[6],
            "customername": header[7],
            "customersite": header[8],

            "invoice_no": header[9],        # ✅ FIXED
            "items": items,
            "itemtotalqty": total_qty,
        }
...
def Return_invoice_print(request):
    ...
    try:
        invoice_return_id = request.GET.get("invoice_return_id")

        if not invoice_return_id:
            return JsonResponse(
                {"error": "invoice_return_id is required"},
                status=400
            )

        # -------------------------------------------------
        # 1️⃣ Fetch data from WHR_INVOICE_RETURN_HISTORY_TBL
        # -------------------------------------------------
        with connection.cursor() as cursor:
            cursor.execute("""
                SELECT
        ...
        if not rows:
            return JsonResponse(
                {"error": "No data found for given INVOICE_RETURN_ID"},
                status=404
            )
        ...
        # -------------------------------------------------
        # 2️⃣ Header data (from first row)
        # -------------------------------------------------
        header = data[0]
        ...
        for idx, row in enumerate(data, start=1):
            qty = row.get('RETURNED_QTY') or 0
            total_qty += qty

            items.append({
                's_no': idx,
                'item_code': row.get('ITEM_CODE'),
                'item_details': row.get('ITEM_DESCRIPTION'),
                'qty': qty,
            })
        # -------- FORMAT DATE --------
        ...
        remmarks = header.get('REMARKS')
         # Default fallback for null/empty values
        remmarks = '' if remmarks in ['null', 'None', None, ''] else remmarks

        # -------------------------------------------------
        # 4️⃣ Context for template
        # -------------------------------------------------
        context = {
            'uniqulastreqno': header.get('INVOICE_RETURN_ID'),
            'date': formatted_date,
            'region': header.get('ORG_NAME'),
            'superuserno': header.get('MANAGER_NO'),
            'superusername': header.get('MANAGER_NAME'),
            'salesmano': header.get('SALESMANO_NO'),
            'salesmanoname': header.get('SALESMAN_NAME'),
            'customerNo': header.get('CUSTOMER_NUMBER'),
            'customername': header.get('CUSTOMER_NAME'),
            'customersite': header.get('CUSTOMER_SITE_ID'),
            'invoiceno': header.get('INVOICE_NUMBER'),
            'remarks': remmarks,
            'itemtotalqty': total_qty,
            'items': items,
        }
...
@csrf_exempt
def Generate_Shipment_dispatch_print(request, shipment_id):
    if request.method != 'GET':
        return JsonResponse({'error': 'Only GET method allowed'}, status=405)
    ...
    try:
        with connections['default'].cursor() as cursor:
            ...
            cursor.execute("""
                SELECT TOP 1
                    ...
                    INVOICE_NO,
                    DL_NUMBER
                FROM WHR_INTER_ORG_DETIALS_TBL
                WHERE SHIPMENT_ID = %s
            """, [shipment_id])
            ...
            if not header:
                return JsonResponse(
                    {'error': f'No data found for SHIPMENT_ID {shipment_id}'},
                    status=404
                )
        ...
        # ---------- CLEAN REMARKS ----------
        remarks = header[14]
        if remarks in ['null', 'None', None, '']:
            remarks = ''

        # ---------- CONTEXT ----------
        context = {
            # Header
            'shipmentid': header[0] or '',
            'shipmentnum': header[1] or '',
            'receiptnum': header[2] or '',
            'date': formatted_date,
            ...
            # From / To Org
            'fromorgcode': header[10] or '',
            'fromorgname': header[11] or '',
            'toorgcode': header[12] or '',
            'toorgname': header[13] or '',

            # Remarks
            'remarks': remarks,

            #Dispatch type
            'dispatch_to': header[15] or '',
            # Customer & Invoice
            'customerNo': header[16] or '',
            'customername': header[17] or '',
            'invoiceno': header[18] or '',
            'dl_number': header[19] or '',
...
def update_active_status_by_shipment_id(request):
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute("""
                UPDATE [BUYP].[dbo].[WHR_INTER_ORG_DETIALS_TBL]
...
def _generate_invoice_return_id_helper(request):
    # Generate a CSRF token
    token = get_token(request)
    ...
    # Construct the final INVOICE_RETURN_ID with padded number
    next_invoice_return_id = f"{prefix}{next_number:01d}"

    # Save to the database
    return INVOICE_RETURN_ID_Models.objects.create(INVOICE_RETURN_ID=next_invoice_return_id, TOCKEN=token)

class GenerateTokenForInvoiceReturnIdView(APIView):    
    def get(self, request, *args, **kwargs):
        new_record = _generate_invoice_return_id_helper(request)
        return Response({
            "INVOICE_RETURN_ID": new_record.INVOICE_RETURN_ID,
            "TOCKEN": new_record.TOCKEN
        }, status=status.HTTP_200_OK)
...
class SaveInvoiceReturnHistory(APIView):
    def post(self, request):
        try:
            ...
            # --- 1. Determine INVOICE_RETURN_ID (Once for the whole batch) ---
            # Check if the first item has an ID. If so, assume it applies/is provided for the batch.
            # Otherwise, generate ONE new ID for all items.
            first_item = data_list[0]
            invoice_return_id = first_item.get("INVOICE_RETURN_ID")
            
            if not invoice_return_id:
                try:
                    new_id_record = _generate_invoice_return_id_helper(request)
                    invoice_return_id = new_id_record.INVOICE_RETURN_ID
                except Exception as e_gen:
                    return Response({"error": f"Failed to generate INVOICE_RETURN_ID: {str(e_gen)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            # -------------------------------------------------------------
            ...
            with transaction.atomic():
                with connection.cursor() as cursor:
                    for data in data_list:
                        ...
                        # Fetch SALESMAN_NAME (Per item lookup - could be optimized further but kept safe for now)
                        cursor.execute("""
                            SELECT TOP 1 NAME 
                        ...
                        """, [salesman_no])
                        result = cursor.fetchone()
                        salesman_name = result[0] if result else ''
            ...
            return Response({
                "message": "Data inserted successfully.",
                "GENERATED_INVOICE_RETURN_ID": invoice_return_id,
...
#                                         ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
#                                     ) AS REMAIN_BALANCE
#                             FROM BUYP.dbo.WHR_INTER_ORG_DETIALS_TBL D
...
#                         D.ITEM_ID,
#                         D.DESCRIPTION AS DISPATCH_DESCRIPTION,
#                         ISNULL(D.QUANTITY_SHIPPED, 0) AS QUANTITY_SHIPPED,
#                         ISNULL(D.QUANTITY_PROGRESS, 0) AS QUANTITY_PROGRESS,
...
#                                 ORDER BY D.ID
#                             ), 0
#                         ) AS FIRST_RECEIVED_QTY,

#                         -- Total progress including current row
#                         ISNULL(
#                             SUM(D.QUANTITY_PROGRESS) OVER (
#                                 PARTITION BY D.SHIPMENT_LINE_ID
#                                 ORDER BY D.ID
...
#                             ORDER BY D.ID DESC
#                         ) AS rn
#                     FROM BUYP.dbo.WHR_INTER_ORG_DETIALS_TBL D WITH (NOLOCK)
#                     LEFT JOIN BUYP.BUYP.ALJE_ITEM_CATEGORIES_CPD_V I
#                         ON D.ITEM_ID = I.ITEM_CODE
#                     WHERE {where_sql}
#                 )
...
#                     *,
#                     CASE
#                         WHEN QUANTITY_SHIPPED - (FIRST_RECEIVED_QTY + TOTAL_PROGRESS) < 0
#                         THEN 0
#                         ELSE QUANTITY_SHIPPED - (FIRST_RECEIVED_QTY + TOTAL_PROGRESS)
...
#                     "DISPATCH_QTY": r["QUANTITY_PROGRESS"],
#                     "REMAINING_QTYt": r["REMAINING_QTY"],  # ✅ Correct & consistent
#                 })
...
def get_InterORG_count(request):
    ...
    with connection.cursor() as cursor:
        cursor.execute("""
            ...
            FROM [BUYP].[dbo].[WHR_INTER_ORG_DETIALS_TBL]
            WHERE WAREHOUSE_NAME = %s
        """, [warehouse_name])
...
def save_dispatch_request(request):
    ...
    try:
        ...
        # Prepare map to update stock later
        stock_update_map = {}
        ...
        with transaction.atomic():
            with connection.cursor() as cursor:
                ...
                for row in tabledatas:
                    ...
                    item_location_code = row.get("ITEM_LOCATION_CODE")
                    item_location_barcode = row.get("ITEM_LOCATION_BARCODE")
                    ...
                    # Build stock_update_map for later
                    key = (item_code, item_location_code, item_location_barcode)
                    stock_update_map[key] = stock_update_map.get(key, 0) + scan_qty

                    # =========================
                    # INSERT SAVED TRUCK DETAILS
                    # =========================
                    for _ in range(scan_qty):
                        cursor.execute("""
                            INSERT INTO WHR_SAVE_TRUCK_DETAILS_TBL (
                                ...
                                Product_code, Serial_No, Udel_id, SCAN_STATUS,
                                manager_no, manager_name, loadman_no, loadman_name,
                                item_location_code, item_location_barcode
                            )
                            VALUES (
                                %s, %s, %s,
                                %s, %s,
                ...
                # =========================
                # UPDATE STOCK TO OnProgress
                # =========================
                for (item_code, loc_code, loc_barcode), final_to_insert in stock_update_map.items():
                    ...
                    logger.info(
                        f"Stock updated → Item:{item_code}, Qty:{final_to_insert}, Pick:{pick_id}"
                    )

        return JsonResponse({
            "status": "success",
            "message": "Data saved and stock updated successfully",
            "req_no": req_id,
            "pick_id": pick_id,
            "customer_no": customer_no,
            "customer_name": customer_name,
        ...
        logger.exception("Error in save_dispatch_request")
        return JsonResponse({"status": "error", "message": str(e)}, status=500)
...
#                                 Item_code, Item_detailas, DisReq_Qty, Send_qty,
#                                 Product_code, Serial_No, Udel_id, SCAN_STATUS,
#                                 manager_no, manager_name, loadman_no, loadman_name, item_location_code, item_location_barcode
#                             )
#                             VALUES (
#                                 %s, %s, %s,
#                                 %s, %s,
...
#                             product_code, undel_id,
#                             manager_no, manager_name,
#                             manager_no, manager_name ,item_location_code,item_location_barcode
#                         ])
...
def truck_scan_view(request):
    ...
    try:
        ...
        # -----------------------------------------------------------
        # MODIFICATION: Auto-generate DISPATCH_ID via existing view
        # -----------------------------------------------------------
        try:
            # Call the view to generate ID
            # Note: create_deliverid_code is a DRF @api_view, so it returns a Response object
            generated_resp = create_deliverid_code(request)
            dispatch_id = generated_resp.data.get("DELIVERY_ID")
            
            if not dispatch_id:
                raise Exception("Generated Delivery ID is null")
                
        except Exception as e_gen:
            return JsonResponse({"error": f"Failed to generate DISPATCH_ID: {str(e_gen)}"}, status=500)
        req_id            = data.get("REQ_ID")
        date              = data.get("DATE")
        warehouse         = data.get("PHYSICAL_WAREHOUSE")
        org_id            = data.get("ORG_ID")
        ...
        physical_loaded_from        = data.get("PHYSICAL_LOADED_FROM")
        ...
        insert_sql = """
            INSERT INTO [BUYP].[dbo].[WHR_TRUCK_SCAN_DETAILS] (
                ...
                CREATION_DATE, CREATED_BY, CREATED_IP, CREATED_MAC,
                LAST_UPDATE_DATE, LAST_UPDATED_BY, LAST_UPDATE_IP, FLAG,
                DELIVERY_DATE, UNDEL_ID, ATTRIBUTE1,LOADMAN_NO , LOADMAN_NAME,ITEM_LOCATION_CODE, ITEM_LOCATION_BARCODE,WAYBILL_NO,PHYSICAL_LOADED_FROM
            ) VALUES (
                %s,%s,%s,%s,%s,
                %s,%s,%s,%s,
                %s,%s,%s,%s,
        ...
        for row in tabledata:
            ...
            params_chunk.append((
                ...
                creation_date, created_by, created_ip, created_mac,
                last_update_date, last_updated_by, last_update_ip, flag,
                delivery_date, row.get("Udel_id"), row_attr , row.get("loadman_no"), row.get("loadman_name"),
                row.get("item_location_code"), row.get("item_location_barcode"), waybill_no, physical_loaded_from
            ))
        ...
        with connections["default"].cursor() as cursor:
            cursor.execute(
                "SELECT COUNT(*) FROM [BUYP].[dbo].[WHR_TRUCK_SCAN_DETAILS] WHERE DISPATCH_ID=%s",
                [dispatch_id]
            )
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT [REQ_ID],[PICK_ID],[SALESMAN_NO],[SALESMAN_NAME],
                       ...
                       [SERIAL_NO],[UNDEL_ID],[ITEM_LOCATION_CODE],[ITEM_LOCATION_BARCODE]
                FROM [BUYP].[dbo].[WHR_PICKED_MAN]
            ...
            for row in rows:
                (req_no, pick_id, salesman_no, salesman_name,
                 ...
                 serial_no, udel_id,item_location_code,item_location_barcodecode) = row
                ...
                if total_saved < total_pickman:
                    cursor.execute("""
                        ...
                        ([dispatch_id],[req_no],[pick_id],[salesman_no],[salesman_name],
                         ...
                         [Send_qty],[Product_code],[Serial_No],[Udel_id],[SCAN_STATUS],[item_location_code],[item_location_barcode])
                        VALUES (%s,%s,%s,%s,%s,
                                %s,%s,%s,%s,
                                %s,%s,
                                %s,%s,%s,
                    ...
                    """, [
                        ...
                        '1', product_code, serial_no, udel_id, "PENDING",item_location_code,item_location_barcodecode
                    ])
                    inserted += 1
                else:
                    skipped += 1
...
@csrf_exempt
def Show_button_truck_details_Loadman_Skip_Scan(request):
    if request.method != "POST":
        return JsonResponse({"error": "Only POST method allowed"}, status=405)
    ...
    try:
        ...
        if not reqid or not pickid:
            return JsonResponse(
                {"error": "Both reqid and pickid are required"},
                status=400
            )
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    SELECT
                        ...
                        SALESMAN_NO, SALESMAN_NAME,
                        MANAGER_NO, MANAGER_NAME,
                        PICKMAN_NO, PICKMAN_NAME,
                        CUSTOMER_NUMBER, CUSTOMER_NAME, CUSTOMER_SITE_ID,
                        INVOICE_NUMBER, CUSTOMER_TRX_ID, CUSTOMER_TRX_LINE_ID,
                        LINE_NUMBER, INVENTORY_ITEM_ID, ITEM_DESCRIPTION,
                        TOT_QUANTITY, DISPATCHED_QTY,
                        ITEM_LOCATION_CODE, ITEM_LOCATION_BARCODE
                    FROM BUYP.dbo.WHR_PICKED_MAN
                    WHERE REQ_ID=%s
                      AND PICK_ID=%s
                ...
                for row in rows:
                    ...
                    """, [item_code])
                    prod = cursor.fetchone()
                    product_barcode = prod[0] if prod and prod[0] else '00'

                    # ---- INSERT ONLY BALANCE ----
                    rows_to_insert = total_picked - total_saved
                    ...
                    if total_saved + rows_to_insert == total_picked:

                        # UPDATE PICKED MAN
                        cursor.execute("""
                            UPDATE BUYP.dbo.WHR_PICKED_MAN
                            SET PRODUCT_CODE=%s,
                                SERIAL_NO=%s
                            WHERE REQ_ID=%s
                              AND PICK_ID=%s
                              ...
                              AND SERIAL_NO='empty'
                        """, [
                            product_barcode, 'LSSNSA',
                            req_no, pick_id, udel_id
                        ])
                        updated_picked += cursor.rowcount

                        # UPDATE SAVE TRUCK (ONLY EMPTY ROWS)
                        cursor.execute("""
                            UPDATE BUYP.dbo.WHR_SAVE_TRUCK_DETAILS_TBL
                            SET Product_code=%s,
                                Serial_No=%s
        ...
        return JsonResponse({
            "status": "success",
            "inserted": inserted,
            "picked_updated": updated_picked,
            "save_updated": updated_saved,
            "skipped": skipped,
            "reqid": reqid,
            "pickid": pickid
        }, status=200)
...
#                  disreq_qty, send_qty, product_code,
#                  serial_no, udel_id,
#                  item_location_code, item_location_barcodecode) = row
...
#                 GET_item_product_barcode = (
#                     prod_row[0] if prod_row and prod_row[0] not in (None, '', ' ') else '00'
#                 )
...
#                          [invoice_no],[Customer_trx_id],[Customer_trx_line_id],
#                          [line_no],[Item_code],[Item_detailas],[DisReq_Qty],
#                          [Send_qty],[Product_code],[Serial_No],[Udel_id],
#                          [SCAN_STATUS],[item_location_code],[item_location_barcode])
#                         VALUES (%s,%s,%s,%s,%s,
#                                 %s,%s,%s,%s,
#                                 %s,%s,
#                                 %s,%s,%s,
...
#                         invoice_no, trx_id, trx_line_id,
#                         line_no, item_code, item_details, disreq_qty,
#                         '1', GET_item_product_barcode, 'LSSNSA',
#                         udel_id, "PENDING",
#                         item_location_code, item_location_barcodecode
#                     ])
#                     inserted += 1
...
#                           AND pick_id=%s
#                           AND Udel_id=%s
#                           AND (Product_code='empty' OR Serial_No='emptyserial')
#                     """, [
#                         GET_item_product_barcode,
#                         req_no, pick_id, udel_id
#                     ])
...
def add_shipment_dispatch_update(request):
    ...
    try:
        ...
        if not isinstance(body, list):
            return JsonResponse(
                {"status": "error", "message": "Expected list of shipment rows"},
                status=400
            )

        # --- GENERATE SHIPMENT ID ONCE ---
        try:
            new_shipment_obj = generateshipment_id_helper()
            generated_shipment_id = new_shipment_obj.shipment_id
        except Exception as e_gen:
            return JsonResponse(
                {
                    "status": "error",
                    "message": f"Failed to generate Shipment ID: {str(e_gen)}"
                },
                status=500
            )

        insert_columns = [
            "SHIPMENT_ID", "WAREHOUSE_NAME", "TO_WAREHOUSE_NAME", "SALESMANNO",
            "SALESMANAME", "DATE", "TRANSPORTER_NAME", "DRIVER_NAME",
            "DRIVER_MOBILENO", "VEHICLE_NO", "TRUCK_DIMENSION",
            "LOADING_CHARGES", "TRANSPORT_CHARGES", "MISC_CHARGES",
            "DELIVERYADDRESS", "SHIPMENT_HEADER_ID", "SHIPMENT_LINE_ID",
            "LINE_NUM", "CREATION_DATE", "CREATED_BY", "ORGANIZATION_ID",
            "ORGANIZATION_CODE", "ORGANIZATION_NAME", "SHIPMENT_NUM",
            "RECEIPT_NUM", "SHIPPED_DATE", "TO_ORGN_ID", "TO_ORGN_CODE",
            "TO_ORGN_NAME", "QUANTITY_SHIPPED", "QUANTITY_PROGRESS",
            "QUANTITY_RECEIVED", "UNIT_OF_MEASURE", "ITEM_ID",
            "DESCRIPTION", "FRANCHISE", "FAMILY", "CLASS", "SUBCLASS",
            "SHIPMENT_LINE_STATUS_CODE", "ACTIVE_STATUS", "REMARKS", 'DISPATCH_TO','CUSTOMER_NO','CUSTOMER_NAME','INVOICE_NO','DL_NUMBER'
        ]

        placeholders = ",".join(["%s"] * len(insert_columns))
        sql = f"""
            INSERT INTO [BUYP].[dbo].[WHR_INTER_ORG_DETIALS_TBL]
            ({','.join(insert_columns)})
            VALUES ({placeholders})
        """
        ...
        with connection.cursor() as cursor:
            for row in body:
                # ✅ INJECT GENERATED SHIPMENT ID PER ROW
                row["SHIPMENT_ID"] = generated_shipment_id

                insert_values = [row.get(col) for col in insert_columns]
                cursor.execute(sql, insert_values)

                # --- UPDATE LOGIC (UNCHANGED) ---
                transfer_type = row.get("TRANSFER_TYPE")
                qty_recent = row.get("UPDATE_QUANTITY")
                    ...
                    return JsonResponse(
                        {
                            ...
                            "message": "Inserted but quantity update failed",
        ...
        return JsonResponse(
            {
                "status": "success",
                "message": "All shipment rows inserted and quantities updated",
                "Shipment_id": generated_shipment_id,
                "rows_inserted": len(body)
            },
            status=201
        )
    ...
    with connection.cursor() as cursor:
        cursor.execute("""
             SELECT
                ...
                LOADMAN_SCAN_SKIP
            FROM BUYP.dbo.WHR_USER_MANAGEMENT            
            WHERE EMP_ROLE LIKE %s
            ORDER BY PHYSICAL_WAREHOUSE, EMP_NAME
        """, ['%WHR SuperUser%'])   # ✅ PASS % INSIDE PARAMETER

        rows = cursor.fetchall()

    for warehouse, username, name, quick_bill_enable, direct_loadman_dispatch in rows:
        ...
        direct_loadman_visible = False
        if direct_loadman_dispatch is not None:
            if isinstance(direct_loadman_dispatch, bool):
                quick_bill_visible = direct_loadman_dispatch
            elif isinstance(direct_loadman_dispatch, int):
                direct_loadman_visible = bool(direct_loadman_dispatch)
            elif isinstance(direct_loadman_dispatch, str):
                direct_loadman_visible = direct_loadman_dispatch.strip().lower() == 'true'
        ...
        data[warehouse].append({
            ...
            "LOADMAN_SCAN_SKIP": direct_loadman_visible
        })
...
@csrf_exempt
def whr_user_management_internal_damage(request):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # 🔹 INTERNAL DAMAGE ENTRY
            if request_type == "internal_damage_entry":
                query = """
                    SELECT
                        PHYSICAL_WAREHOUSE,
                        EMP_USERNAME,
                        EMP_NAME,
                        EMP_ROLE,
                        INTERNAL_DAMAGE_ENTRY
                    FROM BUYP.dbo.WHR_USER_MANAGEMENT
                    WHERE 
                        (EMP_ROLE LIKE '%Pickup%' 
                         OR EMP_ROLE LIKE '%Load%')
                    ORDER BY PHYSICAL_WAREHOUSE, EMP_NAME
                """

            # 🔹 INTERNAL DAMAGE APPROVAL
            elif request_type == "internal_damage_approval":
                query = """
                    SELECT
                        PHYSICAL_WAREHOUSE,
                        EMP_USERNAME,
                        EMP_NAME,
                        EMP_ROLE,
                        INTERNAL_DAMAGE_APPROVAL
                    FROM BUYP.dbo.WHR_USER_MANAGEMENT
                    WHERE 
                        EMP_ROLE LIKE '%WHR SuperUser%'
                    ORDER BY PHYSICAL_WAREHOUSE, EMP_NAME
                """
            else:
                return JsonResponse({
            ...
            for row in rows:
                ...
                # 🔹 Convert NULL to False and 1/0 to Boolean
                for key, value in row_dict.items():
                    if key in ["INTERNAL_DAMAGE_ENTRY", "INTERNAL_DAMAGE_APPROVAL"]:
...
def update_user_access(request):
    try:
        ...
        # ---------------- VALIDATION ----------------
        if employee_id is None or update_type is None or status is None:
            return JsonResponse(
                {"error": "employee_id, update_type and status are required"},
                status=400
            )

        # ---------------- UPDATE TYPE → COLUMN MAP ----------------
        update_column_map = {
            "Quick bill enable": "QUICK_BILL_ENABLE",
            "Loadman Scan Skip": "LOADMAN_SCAN_SKIP",
            
            "INTERNAL_DAMAGE_ENTRY": "INTERNAL_DAMAGE_ENTRY",
            "INTERNAL_DAMAGE_APPROVAL": "INTERNAL_DAMAGE_APPROVAL",
        }

        column_name = update_column_map.get(update_type)
        ...
        # ---------------- NORMALIZE STATUS ----------------
        # Store as nvarchar ('true' / 'false')
        db_value = "true" if bool(status) else "false"
        ...
        with connection.cursor() as cursor:
            query = f"""
                ...
                WHERE EMPLOYEE_ID = %s
            """
            cursor.execute(query, [db_value, str(employee_id)])
...
@require_GET
def get_user_access_status_by_employeeid(request):
    ...
    if row:
        ...
        if isinstance(val1, bool):
            quick_bill_enable = val1
        elif isinstance(val1, int):
            quick_bill_enable = bool(val1)
        elif isinstance(val1, str):
            quick_bill_enable = val1.strip().lower() == 'true'

        # LOADMAN_SCAN_SKIP
        val2 = row[1]
        if isinstance(val2, bool):
            loadman_scan_skip = val2
        elif isinstance(val2, int):
            loadman_scan_skip = bool(val2)
        elif isinstance(val2, str):
            loadman_scan_skip = val2.strip().lower() == 'true'

    # ---------------- RESPONSE ----------------
    return JsonResponse({
        "employee_id": str(employee_id),
        "quick_bill_enable": quick_bill_enable,
        "loadman_scan_skip": loadman_scan_skip
    })
...
def update_admin_quick_bill_visible(request):
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # 🔹 INSERT if employee_id not found
            if cursor.rowcount == 0:
                cursor.execute("""
                    INSERT INTO BUYP.dbo.WHR_EMPLOYEE_ACCESS_TBL
                    (employee_id, Quick_bill_Visible, access_type)
                    VALUES (%s, %s, %s)
                """, [str(employee_id), db_value, access_type])

        return JsonResponse({
            "employee_id": str(employee_id),
            "quick_bill_visible": bool(quick_bill_visible),
            "message": "Quick bill visibility saved successfully"
        })
...
@require_GET
def dispatch_item_details_view(request):
    ...
    with connection.cursor() as cursor:
        ...
        # ------------------------------------------------
        # STEP 1: CHECK IF DISPATCH IS ALREADY CONFIRMED
        # ------------------------------------------------
        cursor.execute("""
            SELECT COUNT(*)
            FROM WHR_TRUCK_SCAN_DETAILS
        ...
        if confirmed_count > 0:
            return JsonResponse(
                {"message": "This dispatch is already confirmed"},
                status=200
            )

        # ------------------------------------------------
        # STEP 2: HEADER DATA (ONLY UNCONFIRMED)
        # ------------------------------------------------
        cursor.execute("""
            SELECT TOP 1
                DISPATCH_ID,
            ...
            FROM WHR_TRUCK_SCAN_DETAILS
            WHERE DISPATCH_ID = %s
              AND ISNULL(CONFIRMED_STATUS, '') <> 'Confirmed'
        """, [dispatch_id])
        ...
        if not header_row:
            return JsonResponse(
                {"error": "No unconfirmed data found for this dispatch_id"},
                status=404
            )
        ...
        # ------------------------------------------------
        # STEP 3: ITEM DETAILS (ONLY UNCONFIRMED)
        # ------------------------------------------------
        cursor.execute("""
            SELECT
                UNDEL_ID,
...
@csrf_exempt
def insert_delivery_confirmation(request):
    if request.method != "POST":
        return JsonResponse({"error": "Only POST allowed"}, status=405)
    ...
    try:
        ...
        if not header or not items:
            return JsonResponse({"error": "Header or items missing"}, status=400)

        # ---------------- HEADER VALUES ----------------
        dispatch_id = header.get("DISPATCH_ID")
        org_id = to_float(header.get("ORG_ID"))
        customer_number = str(header.get("CUSTOMER_NUMBER"))
        salesman_no = str(header.get("SALESMAN_NO"))
        salesman_name = header.get("SALESMAN_NAME")
        remarks = header.get("REMARKS", "")
        created_by = to_float(header.get("STAFF_NO"))
        delivery_confirmed_by = to_float(header.get("DELIVERY_CONFIRMED_BY"))

        dispatch_date = datetime.fromisoformat(
            header.get("DATE").replace("Z", "")
        )
        ...
        with transaction.atomic():
            with connections["default"].cursor() as cursor:
                ...
                # ==================================================
                # 🔴 CHECK DISPATCH ID ALREADY EXISTS
                # ==================================================
                cursor.execute("""
                    SELECT 1
                    FROM BUYP.XXALJEBYP_DELIVERY_CONFIRMATION
                    WHERE DISPATCH_NUMBER = %s
                """, [dispatch_id])
                ...
                if cursor.fetchone():
                    return JsonResponse({
                        "status": "skipped",
                        "dispatch_id": dispatch_id,
                        "message": "Dispatch ID already exists. Insert skipped."
                    }, status=200)

                # ==================================================
                # ✅ INSERT LOOP (ONLY IF NOT EXISTS)
                # ==================================================
                for idx, item in enumerate(items, start=1):

                    undel_id = to_float(item.get("UNDEL_ID"))
                    item_code = item.get("ITEM_CODE")
                    confirmed_qty = to_float(item.get("CONFIRMED_QTY"))
                    dispatch_qty = to_float(item.get("TOTAL_ITEM_QTY"))
                    ...
                    cursor.execute("""
                        SELECT
                            ...
                            INVENTORY_ITEM_ID,
                            INVOICE_NUMBER,
                            SUBSTRING(INVOICE_NUMBER, 2, LEN(INVOICE_NUMBER)) AS INVOICE_ORDER_NO,
                            REFERENCE1,
                            SALESREP_ID
                        FROM BUYP.XXALJE_UNDELIVERED_DATA_BUYP1
                        WHERE UNDEL_ID = %s
                    ...
                    (
                        ...
                        inventory_item_id,
                        invoice_number,
                        invoice_Request_number,
                        ord_reference_no,
                        salesnam_id
                    ) = row

                    cursor.execute("""
                        INSERT INTO BUYP.XXALJEBYP_DELIVERY_CONFIRMATION (
                            UNDEL_ID,
                            ORG_ID,
                            CUSTOMER_ID,
                            CUSTOMER_NUMBER,
                ...
                # ---------------- UPDATE TRUCK SCAN STATUS ----------------
                cursor.execute("""
                    UPDATE WHR_TRUCK_SCAN_DETAILS
...
@require_GET
def unconfirmed_dispatch_list_view(request):
    try:
        page = int(request.GET.get("page", 1))
        rows_per_page = int(request.GET.get("rows_per_page", 10000))
    except ValueError:
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("""
            SELECT COUNT(DISTINCT DISPATCH_ID)
            FROM WHR_TRUCK_SCAN_DETAILS
            WHERE ISNULL(CONFIRMED_STATUS, '') <> 'Confirmed'
        ...
        cursor.execute("""
            SELECT
                ...
                T.TRANSPORT_CHARGES,
                T.MISC_CHARGES,
                FORMAT(T.DATE, 'yyyy-MM-ddTHH:mm:ss') AS DATE,
                T.CUSTOMER_NUMBER,
                T.CUSTOMER_NAME,
                T.CUSTOMER_SITE_ID,
                ISNULL(T.DELIVERY_STATUS, '') AS DELIVERY_STATUS,
                ISNULL(T.TRANSPORT_CHARGES, 0) AS TRANSPORT_CHARGES,
                ISNULL(T.MISC_CHARGES, 0) AS MISC_CHARGES,
                ISNULL(T.LOADING_CHARGES, 0) AS LOADING_CHARGES,
                ISNULL(T.SCAN_PATH, '') AS SCAN_PATH,
                COUNT(*) AS TRUCK_SEND_QTY,
                COUNT(*) AS DISREQ_QTY,
                NULL AS RETAIL_SALESMAN,

                -- ✅ FIXED SUBQUERY
                (
                    SELECT STRING_AGG(D.INVOICE_NO, ',')
                    FROM (
                        SELECT DISTINCT INVOICE_NO
                        FROM WHR_TRUCK_SCAN_DETAILS X
                        WHERE X.DISPATCH_ID = T.DISPATCH_ID
                          AND ISNULL(X.CONFIRMED_STATUS, '') <> 'Confirmed'
                          AND X.INVOICE_NO LIKE %s
                    ) D
                ) AS INVOICE_LISTS

            FROM WHR_TRUCK_SCAN_DETAILS T
            WHERE ISNULL(T.CONFIRMED_STATUS, '') <> 'Confirmed'
              AND T.INVOICE_NO LIKE %s
            GROUP BY
                T.ORG_ID, T.ORG_NAME, T.DISPATCH_ID,
                T.SALESMAN_NO, T.SALESMAN_NAME,
                T.PICK_ID, T.REQ_ID,
                T.DATE,
                T.LOADING_CHARGES,
                T.TRANSPORT_CHARGES,
        ...
        """, [invoice_like, invoice_like, offset, rows_per_page])
...
# -------------------------------------------------------------------------
# NEW OPTIMIZED DISPATCH VIEW (Raw SQL)
# -------------------------------------------------------------------------
@api_view(['GET'])
def get_dispatch_details(request):
    """
    Optimized view to fetch dispatch details and summary using raw SQL.
    Follows strict performance requirements: Database-side aggregation, no Python loop calculations.
   
    Modes:
    1. No Param: Returns List of all headers with aggregated stats nested in 'Item_Details'.
    2. ?dispatch_number=XXXX: Returns List of raw row details (SELECT *) for that dispatch.
    """
    # Check for both lowercase and uppercase parameter to be user-friendly
    dispatch_number = request.GET.get('dispatch_number') or request.GET.get('DISPATCH_NUMBER')
 
    try:
        with connection.cursor() as cursor:
            if not dispatch_number:
                # -------------------------------------------------------
                # MODE 1: NO PARAMETER - RETURN AGGREGATED LIST (NESTED)
                # -------------------------------------------------------
                sql = """
                    SELECT
                        DISPATCH_NUMBER,
                        ...
                        SUM(DISPATCH_QTY) as Total_Dispatch_Qty,
                        SUM(CUS_ACCEPT_QTY) as Total_Customer_Acpt_Qty
                    FROM [BUYP].[BUYP].[XXALJEBYP_DELIVERY_CONFIRMATION]
                    GROUP BY
                        DISPATCH_NUMBER,
                        DISPATCH_DATE,
                        CUSTOMER_NUMBER,
                ...
                if cursor.description:
                    ...
                    for row in rows:
                        row_dict = dict(zip(columns, row))
                        # Return flat structure as requested
                        formatted_results.append(row_dict)
 
                # Return list directly without status wrapper
                return Response(formatted_results, status=status.HTTP_200_OK)
 
            else:
                # -------------------------------------------------------
                # MODE 2: WITH PARAMETER - RETURN DETAILS ONLY (SELECT *)
                # -------------------------------------------------------
                ...
                details_sql = """
                    ...
                    WHERE DISPATCH_NUMBER = %s
                """
                cursor.execute(details_sql, [dispatch_number])
               
                details_data = []
                if cursor.description:
                    columns = [col[0] for col in cursor.description]
                    rows = cursor.fetchall()
                    details_data = [dict(zip(columns, row)) for row in rows]
               
                if not details_data:
                     # Optional: Return 404 or just empty list?
                     # Usually empty list is safer if ID technically valid but no rows,
                     # but if ID doesn't exist, 404 is good.
                     # Let's return empty list or 404 based on preference.
                     # User said "hole deatils get", implies just the data.
                     # Let's return 404 if truly empty to be helpful, or just empty list.
                     # Going with 404 for "Not Found" logic consistency.
                     return Response({
                        "message": "Dispatch Number not found"
                    }, status=status.HTTP_404_NOT_FOUND)
...
# -------------------------------------------------------------------------
# UNDELIVERED INVOICE DETAILS VIEW (Q1 Main -> Q2 Dependent)
# -------------------------------------------------------------------------
@api_view(['GET'])
def get_undelivered_invoice_details(request):
    """
    Fetches undelivered invoice details and related context.
    Param: INVOICE_NUMBER (query param)
    
    Logic:
    1. Query 1: Fetch details for specific INVOICE_NUMBER.
    2. dependent Logic: Extract SALESMAN_NO and CUSTOMER_NUMBER from Q1 result.
    3. Query 2: Fetch related undelivered headers for that Salesman & Customer.
    """
    invoice_number = request.GET.get('INVOICE_NUMBER')
    ...
    try:
        with connection.cursor() as cursor:
            if not invoice_number:
                # ----------------------------------------------------------------
                # MODE 1: NO PARAMETER - RETURN GENERAL LIST (Modified Q2)
                # ----------------------------------------------------------------
                # User requested "q2_sql" to run here.
                # Removing specific Salesman/Customer filters to show all undelivered data.
                ...
                general_sql = """
                    ...
                    FROM [BUYP].[BUYP].[XXALJE_UNDELIVERED_DATA_BUYP1] A
                    WHERE 
                          A.REFERENCE2 = 'REQUEST'
                      AND A.DISPATCH_QTY <> A.QUANTITY
                      AND A.TO_WAREHOUSE = %s
                      AND NOT EXISTS (
                            SELECT 1
                            FROM [BUYP].[dbo].[WHR_CREATE_DISPATCH] B
                            WHERE 
                                  B.SALESMAN_NO     = A.SALESMAN_NO
                              AND B.CUSTOMER_NUMBER = A.CUSTOMER_NUMBER
                              AND B.INVOICE_NUMBER  = A.INVOICE_NUMBER
                              AND B.UNDEL_ID        = A.UNDEL_ID
                      )
                """
                cursor.execute(general_sql, [to_warehouse])
                
                columns = [col[0] for col in cursor.description]
                rows = cursor.fetchall()
                general_data = [dict(zip(columns, row)) for row in rows]
                
                return Response(general_data, status=status.HTTP_200_OK)

            else:
                # ----------------------------------------------------------------
                # MODE 2: WITH PARAMETER - FROM URL (Q1 Main -> Q2 Dependent)
                # ----------------------------------------------------------------
                
                # --- QUERY 1: MAIN INVOICE DETAILS ---
                q1_sql = """
                    SELECT DISTINCT
                            A.[SALESMAN_NO],
                            A.[CUSTOMER_NUMBER],
                            ...
                            A.[RETURN_FLAG]
                        FROM [BUYP].[BUYP].[XXALJE_UNDELIVERED_DATA_BUYP1] A
                        LEFT JOIN BUYP.ALJE_ITEM_CATEGORIES_CPD_V I
                            ON I.INVENTORY_ITEM_ID = A.INVENTORY_ITEM_ID
                        WHERE 
                            A.REFERENCE2 = 'REQUEST'
                        AND A.INVOICE_NUMBER = %s
                        AND A.DISPATCH_QTY <> A.QUANTITY
                ...
                """
                cursor.execute(q1_sql, [invoice_number])
                
                columns_q1 = [col[0] for col in cursor.description]
                rows_q1 = cursor.fetchall()
                
                invoice_details = [dict(zip(columns_q1, row)) for row in rows_q1]
                
                if not invoice_details:
                     return Response({
                        "status": "error",
                        "message": "Invoice Number not found or criteria not met"
                    }, status=status.HTTP_404_NOT_FOUND)

                # Return Invoice Details list directly (Requested to remove 'Related_Context' and 'status')
...
#######################################  AUTO GENERATE CREATE DISPATCH VIEW  ##########################################
                                            


def _generate_next_arg_no_helper():
    """
    Internal helper to generate the next unique ARG Number.
    Format: ARGYYMM01, ARGYYMM02...
    Returns the string ARG Number.
    """
    while True:
        try:
            # 1. Get current Year (YY) and Month (MM)
            ...
            year_str = now.strftime("%y")  # e.g., '25'
            month_str = now.strftime("%m") # e.g., '12'
            prefix = f"ARG{year_str}{month_str}" # e.g., 'ARG2512'

            # 2. Find the latest sequence number for this Month/Year
            last_entry = Uniq_ARG_tbl.objects.filter(
                ARG__startswith=prefix
            ).annotate(
                text_len=Length('ARG')
            ).order_by('-text_len', '-ARG').first()

            if last_entry:
                # Extract the part after the prefix
                last_no = last_entry.ARG
                sequence_str = last_no[len(prefix):] 
                if sequence_str.isdigit():
                    next_seq = int(sequence_str) + 1
                else:
            ...
            # 3. Format the new No with zero-padding (2 digits)
            new_arg_no = f"{prefix}{next_seq:02d}"

            # 4. Attempt to create the record (Handles Race Conditions via unique constraint)
            with transaction.atomic():
                Uniq_ARG_tbl.objects.create(ARG=new_arg_no)
...
@api_view(['GET'])
def get_next_arg_no(request):
    """
    View to generate the next unique ARG Number.
    Uses the helper function.
    """
    try:
        new_arg_no = _generate_next_arg_no_helper()
        return Response({
            "status": "success",
            "ARG": new_arg_no
        }, status=status.HTTP_200_OK)
...
def _fetch_and_group_undelivered_data(to_warehouse):
    """
    Helper function to fetch undelivered data and group it by header.
    Returns: List of grouped header objects with generated REQ_IDs.
    """
    sql = """
        SELECT DISTINCT
              A.[ORG_ID],
              ...
              A.INVOICE_DATE,
              A.[INVOICE_NUMBER],
                ISNULL(NULLIF(A.CUSTOMER_TRX_ID, ''), 0)       AS CUSTOMER_TRX_ID,
                ISNULL(NULLIF(A.CUSTOMER_TRX_LINE_ID, ''), 0) AS CUSTOMER_TRX_LINE_ID,
              A.[LINE_NUMBER],
              I.ITEM_CODE,
              I.DESCRIPTION AS ITEM_DESCRIPTION,
              A.[QUANTITY],
        ...
        WHERE 
              A.REFERENCE2 = 'REQUEST'
          AND A.DISPATCH_QTY <> A.QUANTITY and A.TO_WAREHOUSE = %s
          AND NOT EXISTS (
                SELECT 1
                FROM [BUYP].[dbo].[WHR_CREATE_DISPATCH] B
                WHERE 
    ...
    with connection.cursor() as cursor:
        cursor.execute(sql, [to_warehouse])
        ...
        for row in rows:
            ...
            group_key = (
                data.get('SALESMAN_NO'), 
                data.get('CUSTOMER_NUMBER'), 
                data.get('INVOICE_NUMBER')
            )

            if group_key not in grouped_data:
                arg_value = _generate_next_arg_no_helper()

                grouped_data[group_key] = {
                    "header_level_details": {
                        "ORG_ID": data.get('ORG_ID'),
                        "ORG_NAME": data.get('ORG_NAME'),
                        "PHYSICAL_WAREHOUSE": data.get('TO_WAREHOUSE'),
                        "SALESMAN_NO": data.get('SALESMAN_NO'),
                        "SALESMAN_NAME": data.get('SALESMAN_NAME'),
                        "CUSTOMER_NUMBER": data.get('CUSTOMER_NUMBER'),
                        "CUSTOMER_NAME": data.get('CUSTOMER_NAME'),
            ...
            item = {
                "LINE_NUMBER": data.get('LINE_NUMBER'),
                "INVENTORY_ITEM_ID": data.get('ITEM_CODE'),
                "ITEM_DESCRIPTION": data.get('ITEM_DESCRIPTION'),
                "TOT_QUANTITY": data.get('QUANTITY'),
                "DISPATCH_QTY": data.get('QUANTITY'),
                "DISPATCHED_BY_MANAGER": data.get('QUANTITY'),
                "TRUCK_SCAN_QTY": data.get('QUANTITY'),
                "UNDEL_ID": data.get('UNDEL_ID')
            }
            grouped_data[group_key]["item_details"].append(item)

        return list(grouped_data.values())


@api_view(['GET'])
def get_consolidated_undelivered_details(request):
    """
    Fetches undelivered data, groups it by Header (Salesman+Customer+Invoice),
    generates a unique ARG number for each header, and returns a structured JSON.
    """
    try:
        to_warehouse = request.GET.get('TO_WAREHOUSE')
        response_list = _fetch_and_group_undelivered_data(to_warehouse)
...
@api_view(['POST'])
def create_dispatch_for_Auto_Generate(request):
    """
    Consumes the consolidated undelivered data logic and bulk inserts it into the dispatch table.
    Expects CREATION_DATE, CREATED_BY, and FLAG in the request body.
    """
    try:
        ...
        # User requested capturing IP inside the call? Provided table schema has IP.
        # Let's grab IP best effort.
        created_ip = request.META.get('REMOTE_ADDR')

        # 1. Fetch grouped data (Includes unique REQ_ID generation)
        grouped_data_list = _fetch_and_group_undelivered_data(to_warehouse)
        
        if not grouped_data_list:
             return Response({"status": "error", "message": "No undelivered data found to process."}, status=status.HTTP_404_NOT_FOUND)

        insert_params = []
        
        # 2. Iterate and Prepare Insert Data
        for header_obj in grouped_data_list:
            req_id = header_obj.get('REQ_ID')
            header = header_obj.get('header_level_details', {})
            items = header_obj.get('item_details', [])
            
            for item in items:
                # Map fields to DB columns
                row = (
                    req_id,
                    header.get('PHYSICAL_WAREHOUSE'),
                    header.get('ORG_ID'),
                    header.get('ORG_NAME'),
                    # COMMERCIAL_NO/NAME not in source, leaving None suitable for nullable or default? 
                    # User schema shows them. If not in GET, pass None.
                    0, # COMMERCIAL_NO
                    'Unknown', # COMMERCIAL_NAME
                    header.get('SALESMAN_NO'),
                    header.get('SALESMAN_NAME'),
                    header.get('CUSTOMER_NUMBER'),
                    header.get('CUSTOMER_NAME'),
                    header.get('CUSTOMER_SITE_ID'),
                    header.get('INVOICE_DATE'),
                    header.get('INVOICE_NUMBER'),
                    header.get('CUSTOMER_TRX_ID'),
                    header.get('CUSTOMER_TRX_LINE_ID'),
                    item.get('LINE_NUMBER'),
                    item.get('INVENTORY_ITEM_ID'),
                    item.get('ITEM_DESCRIPTION'),
                    item.get('TOT_QUANTITY'),
                    item.get('DISPATCH_QTY'), # DISPATCHED_QTY in DB?
                    # BALANCE_QTY? Not in JSON. DB col exists. 0 or None? 
                    # Logic says Undelivered means Balance is fully open initially? 
                    # Skipping explicit calculation if not requested. Passing 0 or None.
                    0, 
                    item.get('DISPATCHED_BY_MANAGER'),
                    item.get('TRUCK_SCAN_QTY'),
                    creation_date,
                    created_by,
                    created_ip,
                    # CREATED_MAC?
                    ...
                    None, None, None,
                    flag,
                    # DELIVERYADDRESS? REMARKS? DELIVERY_DATE?
        ...
        if not insert_params:
             return Response({"status": "success", "message": "No items to insert"}, status=status.HTTP_200_OK)

        # 3. Bulk Insert
        insert_sql = """
            INSERT INTO [BUYP].[dbo].[WHR_CREATE_DISPATCH] (
                [REQ_ID],
                [PHYSICAL_WAREHOUSE],
                [ORG_ID],
                [ORG_NAME],
        ...
        return Response({
            "status": "success",
            "message": f"Successfully inserted {len(insert_params)} dispatch rows.",
...
#------------------------------------------------------------------------------------------------------------------------
                                        # Internal Damage Mini Uploaded Segment 
#------------------------------------------------------------------------------------------------------------------------
import json
import boto3
...
class MultiInternal_Damage_ImageUploadView(APIView):
    parser_classes = [MultiPartParser]  # ONLY multipart
    ...
    def post(self, request):
        try:
            ...
            if not raw_data:
                return Response(
                    {"error": "data field is required in form-data"},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            # 🔥 AUTO SET COUNTER (NO SERIAL, NO SCAN)
            set_counter = {}
            ...
            for record in records:
                ...
                if not images:
                    return Response(
                        {"error": f"No images found for key '{image_key}'"},
                        status=status.HTTP_400_BAD_REQUEST
                    )
                ...
                # 🔑 Group key (NO serial)
                group_key = f"{unique_id}|{item_code}|{location_code}|{product_code}"

                # 🔢 Auto SET increment
                if group_key not in set_counter:
                    set_counter[group_key] = 1
                else:
                    set_counter[group_key] += 1

                set_no = set_counter[group_key]
                ...
                for idx, image in enumerate(images, start=1):
                    key = f"{base_path}img_{idx}_{quote(image.name)}"

                    s3.upload_fileobj(
                        image,
                        settings.MINIO_BUCKET_INTERNAL_DAMAGE,
                        key
                    )
...
def save_truck_details_by_req_pick(request):
    reqid = request.GET.get('reqid')
    pickid = request.GET.get('pickid')
    ...
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT 
                ...
                COUNT(*) AS item_count
            FROM WHR_SAVE_TRUCK_DETAILS_TBL
            WHERE req_no = %s AND pick_id = %s and SCAN_STATUS ! = 'Request for Delivery'
            GROUP BY
                req_no, pick_id, Udel_id,
                invoice_no, Item_code, Item_detailas
        """, [reqid, pickid])
...
@csrf_exempt
def update_dispatch_charges(request):
    if request.method != "POST":
        return JsonResponse(
            {"status": "error", "message": "POST method required"},
            status=405
    ...
    try:
        ...
        dispatch_id = body.get("dispatch_id")
        transport_charges = body.get("transport_charges")
        loading_charges = body.get("loading_charges")
        misc_charges = body.get("misc_charges")

        # ✅ Validation
        if not dispatch_id:
            return JsonResponse(
                {"status": "error", "message": "dispatch_id is required"},
                status=400
            )
        ...
        if rows_affected == 0:
            return JsonResponse(
                {"status": "error", "message": "No records found for this dispatch_id"},
                status=404
            )
...
#-----------------------------------------------------------------------------------------------------------------------
# Report For Transportor charges report 
#-----------------------------------------------------------------------------------------------------------------------

@csrf_exempt
def get_unique_truck_scan_details_Report(request):
    """
    Optimized view to retrieve distinct truck scan details with server-side pagination.
    Designed to handle high concurrency (~1000 users) using raw SQL.
    Supports 'page' (default 1) and 'limit' (default 10) query parameters.
    """
    if request.method != 'GET':
        return JsonResponse(
            {'status': 'error', 'message': 'GET method required'},
            status=405
        )
    ...
    try:
        ...
        limit_param = request.GET.get('limit', '10000000000')
        ...
        try:
            ...
            if limit < 1: limit = 10000000000
            if limit > 1000: limit = 10000000000 # Safety cap
        except ValueError:
            limit = 10000000000

        offset = (page - 1) * limit
        
        # 4. Filter Logic & Dynamic Query
        filter_type = request.GET.get('filter_type')
        
        # Base SELECT columns
        # Default view (Dispatch Level)
        select_columns = """
            date,
            DISPATCH_ID AS [DISPATCH NUMBER],
            ORG_NAME AS [PHYSICAL WAREHOUSE],
            PHYSICAL_LOADED_FROM AS [PHYSICAL LOADED WAREHOUSE],
            DELIVERYADDRESS AS [TO DESTINATION],
            CUSTOMER_NUMBER,
            CUSTOMER_NAME,
            TRANSPORTER_NAME,
            WAYBILL_NO,
            TRANSPORT_CHARGES AS [TRANSPORT COST]
        """
        
        # Extended view (Qty/Item Level) if requested
        if filter_type == 'qtywise':
             select_columns = """
                date,
                DISPATCH_ID AS [DISPATCH NUMBER],
                ORG_NAME AS [PHYSICAL WAREHOUSE],
        ...
        if to_date:
            where_conditions.append("date <= %s")
            query_params.append(f"{to_date} 23:59:59")

        # -- Filter: Dispatch ID
        dispatch_id = request.GET.get('dispatch_id')
        if dispatch_id:
            where_conditions.append("DISPATCH_ID = %s")
            query_params.append(dispatch_id)

        # -- Filter: Invoice Number
        invoice_no = request.GET.get('invoice_no')
        if invoice_no:
            where_conditions.append("INVOICE_NO = %s")
            query_params.append(invoice_no)

        # -- Filter: Customer Number
        customer_no = request.GET.get('customer_no')
        if customer_no:
            where_conditions.append("CUSTOMER_NUMBER = %s")
            # Ensure type consistency if needed, but parameter binding handles strings mostly
            query_params.append(customer_no)

        # -- Filter: Salesman Number
        salesman_no = request.GET.get('salesman_no')
        if salesman_no:
            where_conditions.append("SALESMAN_NO = %s")
            query_params.append(salesman_no)
        ...
        # For Count, we need the SAME where clause but strictly count rows
        # We need to respect the DISTINCT nature of the main query to get accurate "page count"
        # However, selecting all columns in subquery for distinct count is heavy.
        # Ideally, we count on the same distinct set.
        count_sql = f"""
            SELECT COUNT(*) FROM (
                SELECT DISTINCT 
                    {select_columns}
